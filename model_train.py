...
def data_process():
    ...
    data = FashionMNIST(root='./data',
                        ...
                        download=True,
                        transform=transform)
    train_data_size, val_data_size = utils.get_train_and_val_data_size(len(data), 0.8)

    train_data, val_data = random_split(data, [train_data_size, val_data_size], generator=utils.set_main_process_seed())

    train_data_loader = DataLoader(dataset=train_data,
                                   batch_size=128,
                                   shuffle=True,
                                   num_workers=3,
                                   generator=utils.set_main_process_seed(),
                                   worker_init_fn=utils.set_sub_process_seed)
...
def train_process(model, train_data_loader, val_data_loader, epochs):
    ...
    torch.save(best_model_wts, './alex_net_model/best_model.pth')
